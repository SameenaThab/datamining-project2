...
print("initial plot ")
library(ggplot2)
ggplot(ktablei, aes(x=X1,y=X2))+geom_point(colour="white", shape=21, size = 4,aes(fill = factor(Groups)))+scale_fill_manual(values=c("blue", "red"))
ktable<-ktablei
```
#Q5-b
```{r}
...
centroid2<-c(cenx2,ceny2,4)
print(centrod2)
temp<-rbind(ktable,centroid1)
temp<-rbind(temp,centroid2)
...
cenx1=cenx1/nrow(g11)
ceny1=ceny1/nrow(g11)
centrod1<-data.frame(c(cenx1),c(ceny1))
...
cenx2=cenx2/nrow(g22)
ceny2=ceny2/nrow(g22)
centrod2<-data.frame(c(cenx2),c(ceny2))
